...
st.caption("Streamlit app with multi-vendor PSW logic: vendor matching to PlanDetailTimeline decides main vs sub vendor, even when both vendors are in one PSW file.")
...
with col2:
    due_files = st.file_uploader(
        ...
        help="Upload order matters: first file = main/default vendor transit; second file = sub/other vendor transit. If only one file is uploaded, both vendor groups use the same transit.",
    )

if due_files:
    st.info("DueDateCalc order: " + " | ".join([f"{i+1}. {f.name}" for i, f in enumerate(due_files)]))
    if len(due_files) == 1:
        st.caption("Only one DueDateCalc uploaded. Main and sub vendors will use the same warehouse transit time.")
    elif len(due_files) > 2:
        st.warning("Only the first two DueDateCalc files are used: 1st = main/default, 2nd = sub/other vendor. Extra files are ignored.")
else:
    st.info("Upload at least one DueDateCalc. The first file is main/default transit; the optional second file is sub/other vendor transit.")

psw_files = st.file_uploader(
    "PSW / Production Schedule raw CSV files",
    type=["csv"],
    accept_multiple_files=True,
    help="One file can contain both main and sub vendors. Vendor matching to PlanDetailTimeline decides role; second/subsequent files are forced as other vendor(s).",
)

if psw_files:
    st.info("PSW order: " + " | ".join([f"{i+1}. {f.name}" for i, f in enumerate(psw_files)]))
    if len(psw_files) == 1:
        st.caption("Only one PSW file uploaded. If it contains multiple vendors, the app will still split main vs sub vendor by matching PSW Vendor # to the PlanDetailTimeline Vendor code.")
else:
    st.info("Upload at least one PSW / Production Schedule CSV. A single PSW file may contain both main and sub vendors.")
...
ready = plan_file is not None and bool(due_files) and bool(psw_files)
run_clicked = st.button("Run Full Flow", type="primary", disabled=not ready)

if not ready:
    st.info("Please upload PlanDetailTimeline, at least one DueDateCalc, and at least one PSW / Production Schedule CSV before running.")
    ...
    try:
        with st.spinner("Processing full multi-vendor Destination Change flow..."):
            with tempfile.TemporaryDirectory() as tmp:
                ...
                due_paths = []
                for i, uploaded_due in enumerate(due_files[:2], start=1):
                    due_safe_name = f"due_{i}_{uploaded_due.name}"
                    due_path_i = tmp_path / due_safe_name
                    due_path_i.write_bytes(uploaded_due.getvalue())
                    due_paths.append(str(due_path_i))
                due_path = due_paths[0]
                other_due_path = due_paths[1] if len(due_paths) > 1 else None
                ...
                final_path = process_files(
                    ...
                    other_due_date_calc_xlsx=other_due_path,
        ...
        st.caption(
            ...
            f"Offset mode: {offset_mode} | DueDateCalc files used: {min(len(due_files), 2)} | PSW files: {len(psw_files)} | Priority rules: {len(priority_rules)}"
        )
    except Exception as exc:
