...
def load_psw_vendor_supply(
    ...
    If no vendor-specific transit column exists in PSW, Vendor Transit Weeks falls back to Warehouse Offset Weeks,
    which makes Adjusted Supply Week equal to the PSW ETD Week.
    """
    detail_frames = []
    debug_rows = []
    for file_order, path in enumerate(psw_csv_paths or [], start=1):
        ...
        for wk, col in week_cols:
            ...
            nonzero["Vendor Transit Weeks"] = [
                find_transit_weeks_in_row(r, int(r["Warehouse Offset Weeks"])) for _, r in nonzero.iterrows()
            ]
            ...
            rows.append(nonzero[[
                ...
                "Vendor Transit Weeks", "Warehouse Offset Weeks", "Adjusted Supply Week", "Adjusted Supply Week Text"
            ]])
...
def process_files(
    ...
    psw_supply_detail, psw_read_debug = load_psw_vendor_supply(all_psw_paths, target_week, current_week, offset_map)
